            ...
            <div className="space-y-6">
              ...
              <div className="rounded-[24px] border border-[#eadfce] bg-white p-5">
                ...
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-start">
                  <p className="min-w-0 flex-1 break-all font-mono text-sm text-[#102033]">
                    {receiptQuery.data.hash}
                  </p>
                  <button
                    type="button"
                    onClick={handleCopyHash}
                    aria-label={copiedHash ? 'Transaction hash copied' : 'Copy transaction hash'}
                    title={copiedHash ? 'Copied!' : 'Copy transaction hash'}
                    className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full border border-[#d7eadf] px-3 py-2 text-xs font-bold uppercase tracking-widest text-[#1f8f55] transition-colors hover:bg-[#eef8f2] focus:outline-none focus:ring-2 focus:ring-[#1f8f55] focus:ring-offset-2"
                  >
